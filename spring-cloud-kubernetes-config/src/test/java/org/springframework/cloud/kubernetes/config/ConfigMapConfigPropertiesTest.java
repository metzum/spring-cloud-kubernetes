...
public class ConfigMapConfigPropertiesTest {
	...
	public void testDefaultNameAndNamespace() {
		...
		List<ConfigMapConfigProperties.NormalizedSource> normalizedSources = props
				.determineSources();
		assertThat(normalizedSources).hasSize(1).first()
				.hasFieldOrPropertyWithValue("name", defaultName)
				.hasFieldOrPropertyWithValue("namespace", defaultNamespace);
	}

	@Test
	public void testNoLabels() {
		props.setSources(Collections.singletonList(
				new ConfigMapConfigProperties.Source("nameValue", "namespaceValue")));
		List<ConfigMapConfigProperties.NormalizedSource> normalizedSources = props
				.determineSources();
		assertThat(normalizedSources).hasSize(1).first()
	...
	public void testLabelName() {
		props.setSources(Arrays.asList(
				new ConfigMapConfigProperties.Source(null, null, "labelNameValue1"),
				new ConfigMapConfigProperties.Source("nameValue", null,
						"labelNameValue2"),
				new ConfigMapConfigProperties.Source(null, "namespaceValue",
						"labelNameValue3")));
		List<ConfigMapConfigProperties.NormalizedSource> normalizedSources = props
				.determineSources();
		...
		assertThat(normalizedSources).element(1)
				.hasFieldOrPropertyWithValue("name", "nameValue")
				.hasFieldOrPropertyWithValue("namespace", defaultNamespace)
	...
	public void testLabelValue() {
		props.setSources(Collections.singletonList(new ConfigMapConfigProperties.Source(
				null, null, "labelNameValue=labelValueValue")));
		List<ConfigMapConfigProperties.NormalizedSource> normalizedSources = props
				.determineSources();
		assertThat(normalizedSources).hasSize(1).first()
